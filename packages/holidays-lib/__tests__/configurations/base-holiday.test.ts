...
describe.each([
  ['invalid.cycle-type.six-years.missing-from', ErrorKeys.CYCLE_TYPE_REQUIRES_VALID_FROM],
  ['invalid.cycle-type.two-years.missing-from', ErrorKeys.CYCLE_TYPE_REQUIRES_VALID_FROM],
  ['invalid.from-alphanumeric', ErrorKeys.VALID_FROM_INVALID],
  ['invalid.holiday-status-value', ErrorKeys.HOLIDAY_STATUS_INVALID],
  ['invalid.key-empty', ErrorKeys.KEY_MISSING],
  ['invalid.to-alphanumeric', ErrorKeys.VALID_TO_INVALID],
  ['invalid.to-lt-from', ErrorKeys.VALID_TO_BEFORE_VALID_FROM],
  ['invalid.cycle-type.five-years.missing-from', ErrorKeys.CYCLE_TYPE_REQUIRES_VALID_FROM],
  ['invalid.cycle-type.four-years.missing-from', ErrorKeys.CYCLE_TYPE_REQUIRES_VALID_FROM],
  ['invalid.cycle-type.value', ErrorKeys.CYCLE_TYPE_INVALID]
])('invalid base holiday configurations', (fileName: string, expected: ErrorKeys) => {
  const file = path.join(__dirname, `${dataRoot}/${fileName}.json`);
  const configuration = new ConfigurationFactory().loadByFileName(file);
